...
export let PeriodicNoteConfig: PeriodicNoteSettings | undefined | null =
  undefined;
...
export function getPeriodicConfig(app: App) {
  // Access the plugin instance safely using the app object
  const pluginInstance = (app as any).plugins?.plugins["periodic-notes"];
  const settings = pluginInstance?.settings as PeriodicNoteSettings;
  return settings;
}

export type PeriodicType = "daily" | "weekly" | "monthly" | "yearly";

export function getPathByMoment(
  app: App,
  type: PeriodicType,
  date: moment.Moment,
): string {
  if (PeriodicNoteConfig === undefined) {
    PeriodicNoteConfig = getPeriodicConfig(app);
  }
...
export function getPeriodic(app: App, file: TFile): PeriodicReturn | null {
...
function _getPeriodicType(app: App, file: TFile): PeriodicReturn | null {
  ...
  for (const type of checkTypes) {
    ...
    const format = PeriodicNoteConfig
      ? PeriodicNoteConfig[type].format || defaultFormat[type]
      : defaultFormat[type];
...
export async function openOrCreate(
  ...
  if (
    PeriodicNoteConfig &&
    PeriodicNoteConfig[type] &&
    PeriodicNoteConfig[type].template
  ) {
  ...
  const newFile = app.vault.getFileByPath(path);
  if (newFile) {
